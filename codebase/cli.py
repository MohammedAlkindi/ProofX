...
def _cmd_falsify(args: argparse.Namespace) -> int:
    ...
    for label, key in [
        ...
        for i, e in enumerate(entries[: args.top_k], 1):
            print(f"    #{i}  n={e.candidate:>12,}  score={e.near_miss_score:.4f}"
...
def _cmd_collatz(args: argparse.Namespace) -> int:
    _setup_logging(args.log_level, args.log_file)
    from codebase.CollatzX.Pipeline.pipeline import CollatzPipeline  # type: ignore[import]
    print(f"Running CollatzX pipeline  n_start={args.start}  n_end={args.end}")
    pipeline = CollatzPipeline()
    pipeline.run(args.start, args.end)
    return 0


def _build_collatz(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser("collatz", help="CollatzX pipeline")
    _add_common_args(p)
    p.add_argument("--start", type=int, default=1)
    p.add_argument("--end", type=int, default=10_000)
    p.set_defaults(func=_cmd_collatz)
...
def _cmd_goldbach(args: argparse.Namespace) -> int:
    _setup_logging(args.log_level, args.log_file)
    from codebase.GoldbachX.PartitionEnumerator.PartitionEnumerator import PartitionEnumerator  # type: ignore[import]
    print(f"Running GoldbachX partition enumerator  limit={args.limit}")
    enum = PartitionEnumerator(limit=args.limit)
    enum.run()
    return 0


def _build_goldbach(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser("goldbach", help="GoldbachX partition analysis")
    _add_common_args(p)
