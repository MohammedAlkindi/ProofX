(() => {
  ...
  const PRIME_COUNT_LIMIT = 500_000;
  const input = byId<HTMLInputElement>("riemann-input");
  const button = byId<HTMLButtonElement>("riemann-run");
  const metrics = byId<HTMLDivElement>("riemann-metrics");
  const chart = byId<HTMLDivElement>("riemann-prime-chart");
  const output = byId<HTMLPreElement>("riemann-result");
  ...
  function millerRabin(n: number): boolean {
    if (n < 2) return false;
    if ([2, 3, 5, 7, 11, 13].includes(n)) return true;
  ...
  function nearestPrimes(n: number): { prev: number | null; next: number | null } {
    ...
    for (let candidate = n + 1; candidate <= n + 10_000; candidate += 1) {
      if (millerRabin(candidate)) {
        next = candidate;
        break;
      }
  ...
  function primePrefix(limit: number): Uint32Array {
  ...
  function primeCountSeries(n: number): PrimeSeries {
  ...
  function renderMetrics(n: number, isPrime: boolean, series: PrimeSeries, approxAtN: number): void {
    metrics.innerHTML = [
  ...
  function renderPrimeChart(n: number, series: PrimeSeries): void {
    ...
    const toX = (x: number): number => pad + ((x - 2) / Math.max(1, xMax - 2)) * (width - pad * 2);
    const toY = (y: number): number => height - pad - (y / yMax) * (height - pad * 2);
  ...
  function linePath(points: Point[], toX: (x: number) => number, toY: (y: number) => number): string {
    return points
  ...
  function renderPreview(
    n: number,
    isPrime: boolean,
    near: { prev: number | null; next: number | null },
    series: PrimeSeries,
    approxAtN: number,
  ): void {
