...
(() => {
    ...
    const tableContainer = byId("table-container");
    const visualPanel = byId("visual-panel");
    const summaryPanel = byId("summary-panel");
    const filterConjecture = byId("filter-conjecture");
    const filterStrategy = byId("filter-strategy");
    const candidateSearch = byId("candidate-search");
    const topKSelect = byId("top-k");
    const minScore = byId("min-score");
    const minScoreValue = byId("min-score-val");
    const featureSelect = byId("feature-select");
    ...
    function setEntries(entries, source) {
    ...
    function parseLedgerText(text, source) {
        ...
        if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
            try {
                return normalizePayload(JSON.parse(trimmed), source);
            }
            catch {
                return [];
            }
        ...
        for (const line of trimmed.split(/\r?\n/)) {
            ...
            try {
                const normalized = normalizeEntry(JSON.parse(line), source);
                if (normalized) {
    ...
    function normalizePayload(payload, source) {
        if (Array.isArray(payload)) {
            return payload.flatMap((item) => normalizeEntry(item, source) ?? []);
        }
        if (!isMap(payload)) {
            return [];
        }
        ...
        const groupedEntries = grouped.flatMap(([conjecture, rows]) => {
            return Array.isArray(rows)
                ? rows.flatMap((row) => normalizeEntry(row, source, conjecture) ?? [])
                : [];
        });
        const arrayEntries = ["entries", "ledger", "records"].flatMap((key) => {
            const rows = payload[key];
            return Array.isArray(rows)
                ? rows.flatMap((row) => normalizeEntry(row, source) ?? [])
                : [];
        });
    ...
    function normalizeEntry(raw, source, fallbackConjecture = "unknown") {
        if (!isMap(raw)) {
            return null;
        }
    ...
    function updateStats(filtered, visible) {
    ...
    function renderHistogram(entries) {
        ...
        const bars = bins
            ...
            .join("");
        byId("score-histogram").innerHTML = svgWrap(`${bars}<text x="26" y="205">score bucket lower bound</text>`, "0 0 640 220");
    }
    function renderCandidateMap(entries) {
    ...
    function refreshStrategyFilter() {
    ...
    function refreshFeatureSelect() {
        const keys = new Set(["near_miss_score"]);
    ...
    function conjecturePill(conjecture) {
