...
- This file governs the ProofX root: `packages/python/codebase/`, `tests/`, `docs/`, `src/`, `scripts/`, `assets/`.
...
- One logical change per commit. Don't bundle a `packages/python/codebase/` engine change with a `src/` site change, or a ProofX-root change with a `packages/germinal/` change.
...
- Install/check the dev toolchain with `pip install -r requirements.txt -r requirements-dev.txt -e .` when a clean environment is needed.
- Run `pytest`, `ruff check .`, `ruff format --check .`, and `mypy packages/python/codebase` — all four, not just tests. Never commit without them passing.
