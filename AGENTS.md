...
- This file governs the ProofX root: `codebase/`, `tests/`, `docs/`, `public/`, `src/`, `scripts/`, `assets/`.
...
- One logical change per commit. Don't bundle a `codebase/` engine change with a `public/`/`src/` site change, or a ProofX-root change with a `packages/germinal/` change.
