...
> **Note**: This file is the original context document written for Codex-based workflows. It predates the current Claude-based implementation. For the current developer guide, see [CLAUDE.md](../CLAUDE.md) in the repo root.

Context for Codex working in this repo. Read this before touching anything.

## What this is

Germinal generates mathematical conjectures via Codex, formalizes them in Lean 4, attempts automated proofs, and commits every experiment as a reproducible Git snapshot. Backend is FastAPI + Celery + Postgres/SQLite, frontend is Next.js + Tailwind. Full pipeline: generate → formalize → verify → snapshot.
...
No live Lean install or Anthropic API key is required — all Codex API calls and all `subprocess`/`lake` calls are mocked. Every new module must have a corresponding test file; `tests/test_symbolic_counterexample.py` is the reference example.

## Conventions

- Lint: `ruff check src/ api/` and `ruff format --check src/ api/`. Run before considering anything done.
- Codex API calls use prompt caching (`cache_control: ephemeral`) on static system/instruction blocks — preserve this pattern when adding new Codex calls, don't cache dynamic per-request content.
